...
void attachable_hud_item::setup_firedeps(firedeps& fd)
{
	...
	if (m_measures.m_prop_flags.test(hud_item_measures::e_fire_point))
	{
		...
		//KRodin придумал костыль. Из-за того, что fire_point расположен сильно впереди ствола, попробуем точку вылета пули считать от позиции fire_point.z == -0.7, т.е. ближе к актору, чтобы нельзя было стрелять сквозь стены.
		if (m_measures.useCopFirePoint)
		{
			const Fvector m_shoot_point_offset{ m_measures.m_fire_point_offset.x, m_measures.m_fire_point_offset.y, -0.7f };
